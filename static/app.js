...
[explanation: OSPF has an admin distance of 110. EIGRP is 90, RIP is 120.]</div>
    ...
    render();
       ...
       // Add this to your app.js - IMPROVED REVIEW RENDERING
...
function renderReview() {
    ...
    const html = state.currentQuiz.questions.map((q, i) => {
        ...
        return `
            <div class="card" style="padding:1.5rem;margin-bottom:1.5rem;border-left:4px solid ${correct ? 'var(--success)' : 'var(--error)'}">
                <div class="flex items-center gap-md" style="margin-bottom:1rem">
                    <span style="font-size:2rem">${correct ? '✅' : '❌'}</span>
                    <div style="flex:1">
                        <div class="flex items-center gap-sm" style="margin-bottom:0.25rem">
                            <span class="badge ${correct ? 'badge-success' : 'badge-error'}">${correct ? 'Correct' : 'Incorrect'}</span>
                            <span class="text-sm text-muted">Question ${i + 1}</span>
                        </div>
                        <h3 style="font-size:1.125rem;margin:0">${escapeHtml(q.question)}</h3>
                    </div>
                </div>
                ...
                ${q.code ? `
                    <div class="code-block" style="margin-bottom:1rem">
                        ...
                        <pre class="code-body"><code>${escapeHtml(q.code)}</code></pre>
                    </div>
                ` : ''}
                
                ${q.image ? `
                    <img src="${escapeHtml(q.image)}" alt="Question image" style="max-width:100%;max-height:200px;border-radius:var(--radius-md);margin-bottom:1rem">
                ` : ''}
                
                <div style="margin-bottom:1rem">
                    ${q.type === 'choice' ? q.options.map((opt, j) => {
                        const isU = ua.includes(j);
                        const isC = q.correct.includes(j);
                        let cls = 'option-btn';
                        let icon = '';
                        
                        if (isC) {
                            cls += ' correct';
                            icon = '<span class="badge badge-success">✓ Correct Answer</span>';
                        } else if (isU && !isC) {
                            cls += ' incorrect';
                            icon = '<span class="badge badge-error">✗ Your Answer</span>';
                        }
                        
                        return `
                            <button class="${cls}" disabled style="margin-bottom:0.5rem;cursor:default">
                        ...
                        <div class="card" style="padding:1rem;background:var(--bg-tertiary)">
                            <p class="text-sm text-muted" style="margin-bottom:0.75rem;font-weight:600">Correct order:</p>
                            ${q.correct.map((ci, pos) => {
                                const userCorrect = ua[pos] === ci;
                                return `
                                    <div style="padding:0.5rem;margin-bottom:0.5rem;background:${userCorrect ? 'var(--success-light)' : 'var(--error-light)'};border-radius:var(--radius-sm);display:flex;align-items:center;gap:0.5rem">
                                        <span style="font-weight:600">${pos + 1}.</span>
                                        <span style="flex:1">${escapeHtml(q.options[ci])}</span>
                                        <span>${userCorrect ? '✓' : '✗'}</span>
                                    </div>
                                `;
                            }).join('')}
                        </div>
                ...
                ${q.explanation ? `
                    <div class="explanation-box">
                        <p class="font-semibold" style="margin-bottom:0.5rem">💡 Explanation</p>
    ...
    return `
        <nav class="navbar">
            <div class="container">
                <div class="navbar-inner">
                    <button onclick="state.view='results';render()" class="btn btn-ghost">← Back to Results</button>
                    <div style="text-align:center">
                        <h2 style="font-size:1.125rem;margin-bottom:2px">Review Answers</h2>
                        <p class="text-xs text-muted">${score}/${total} correct</p>
                    </div>
                    <div></div>
                </div>
            </div>
        ...
        <main style="padding:2rem 0 4rem">
            <div class="container" style="max-width:900px">
                ...
                <div class="flex gap-md" style="margin-top:2rem">
                    <button onclick="startQuiz(${state.currentQuiz.id},{studyMode:false})" class="btn btn-accent flex-1">
                        🔄 Try Again
                    </button>
                    <button onclick="state.view='library';render()" class="btn btn-ghost flex-1">
                        ← Back to Library
                    </button>
                </div>
            </div>
