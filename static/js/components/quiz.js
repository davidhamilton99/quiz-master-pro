    ...
    return `<div class="quiz-page">
        ...
        <main class="quiz-main"><div class="quiz-content">
            <div class="question-header">
                <div class="question-num">Question ${state.currentQuestionIndex + 1} ${q.type !== 'choice' ? `<span class="badge badge-primary">${q.type}</span>` : ''} ${state.flaggedQuestions.has(state.currentQuestionIndex) ? '<span class="badge badge-warning">Flagged</span>' : ''}</div>
...
export async function startQuiz(id, opts = {}) {
    ...
    try {
        ...
        setState({ view: 'quiz', currentQuiz: { id, title: quiz.title, questions }, currentQuestionIndex: 0, answers, studyMode: opts.study || false, timerEnabled: opts.timer || false, timerMinutes: opts.minutes || 15, timeRemaining: opts.timer ? (opts.minutes || 15) * 60 : 0, showAnswer: false, flaggedQuestions: new Set(), streak: 0, maxStreak: 0 });
...
export function checkAnswer() {
    ...
    const correct = q.type === 'ordering' ? JSON.stringify(ans) === JSON.stringify(q.correct) : new Set(ans).size === new Set(q.correct).size && ans.every(a => q.correct.includes(a));
...
export function nextQ() { const s = getState(); if (s.currentQuestionIndex < s.currentQuiz.questions.length - 1) { setState({ currentQuestionIndex: s.currentQuestionIndex + 1, showAnswer: false }); saveQuizProgress(); } }
export function prevQ() { const s = getState(); if (s.currentQuestionIndex > 0) setState({ currentQuestionIndex: s.currentQuestionIndex - 1, showAnswer: false }); }
...
export function exitQuiz() { stopTimer(); saveQuizProgress(); setState({ view: 'library', currentQuiz: null }); showToast('Progress saved', 'info'); }
